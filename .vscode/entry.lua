
local production = true --:boolean

require("eom_main")
--require("df_politics_main")



require("export_helpers__eom_main_events")
require("export_helpers__eom_battles")
require("export_helpers__eom_core")
require("export_helpers__eom_first_turn")
require("export_helpers__eom_plot_events")
require("export_helpers__eom_vlad_civil_wars")
require("export_helpers__eom_full_loyalty_events")
require("export_helpers__eom_cult_happiness")
require("export_helpers__eom_capitulation")
require("export_helpers__eom_casus_belli")
require("export_helpers__eom_elector_taxation")
require("export_helpers__eom_elector_diplomacy")
require("export_helpers__eom_ui")
require("export_helpers__eom_chaos_invasion")
require("export_helpers__eom_revival")
require("export_helpers__eom_volkmar_unlock")
require("export_helpers__eom_civil_war_system")


require("compat_scripts/export_helpers__eom_zarkis_compat")